        ...
        //location.reload();
            ...
            if(get_option('wc_settings_quote_on_cart_select') === "true") {
                ...
                var $quote_on_cart_form = '<form method="post" action="' + window.location.href + '">';
                    ...
                    $quote_on_cart_form += '<input type="submit" name="_cart_to_quote" class="button cart_to_quote_submit" value="<?php if(!empty(get_option('wc_settings_quote_button_text'))){ $btn = get_option('wc_settings_quote_button_text'); } else { $btn = 'Build a Quote'; }echo __( $btn, WATQ); ?>" />';
...
function watq_from_cart_to_quote() {
    ...
    if(is_object($woocommerce) || is_array($woocommerce)) {
        foreach($woocommerce->cart->get_cart() as $item_key=> $item_value) {
            ...
            if($item_value['data']->get_type() == "variation"){
